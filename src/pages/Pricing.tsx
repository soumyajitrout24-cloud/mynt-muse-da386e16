...
const fallbackPlans = [
  { id: "1", name: "Executive Hour", subtitle: "Signature Introduction", description: "Refined, short-duration private engagement, ideal for business travelers or tight schedules.", features: ["Elegant, polished presence", "Discreet and professional interaction"], tier: "silver" },
  { id: "2", name: "Elite Experience", subtitle: "Extended Engagement", description: "For clients who prefer unhurried time and elevated attention.", features: ["Relaxed and immersive engagement", "Perfect for fine dining, private meetings, or exclusive evenings", "Enhanced personalization"], tier: "gold" },
  { id: "3", name: "Royal Evening", subtitle: "Luxury Indulgence", description: "Complete, uninterrupted experience tailored for discerning individuals.", features: ["Premium time allocation", "Elevated presence and attention to detail", "Suitable for high-profile gatherings and private stays"], tier: "platinum" },
];

const fallbackAddOns = ["Priority scheduling", "Advance profile reservation", "Customized plan structuring", "Private long-term arrangements"];
...
const Pricing = () => {
  ...
  const addOnItems = addOns || fallbackAddOns;
      ...
      <div className="container mx-auto max-w-5xl">
        <FadeInSection>
          <div className="text-center mb-10 md:mb-16">
            ...
            <h1 className="font-display text-2xl sm:text-3xl md:text-5xl tracking-wider text-primary">Pricing & Plans</h1>
            ...
            <p className="font-elegant text-xs sm:text-sm md:text-base text-primary/45 mt-4 max-w-lg mx-auto leading-relaxed">
              At Mynt Girlfriend, pricing reflects exclusivity, discretion, and uncompromising standards.
            </p>
          </div>
        </FadeInSection>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 items-stretch">
          {items.map((plan, i) => (
            <FadeInSection key={plan.id} delay={i * 0.15}>
              <div className={`relative flex flex-col h-full rounded-2xl p-5 sm:p-6 md:p-8 border transition-all duration-500 hover:-translate-y-1 ${
                plan.tier === "platinum" ? "border-primary/60 bg-card shadow-gold" : plan.tier === "gold" ? "border-primary/40 bg-card" : "border-primary/20 bg-card"
              }`}>
                ...
                <p className="font-elegant text-xs md:text-sm text-primary/40 mb-4 md:mb-6 leading-relaxed">{plan.description}</p>
                <div className="gold-divider w-full mb-4 md:mb-6" />
                <ul className="space-y-3 md:space-y-4 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-center gap-3 p-2 rounded-lg transition hover:bg-primary/5">
                      <Check className={`w-4 h-4 md:w-5 md:h-5 flex-shrink-0 ${plan.tier === "gold" ? "text-gold" : "text-primary/55"}`} />
        ...
        <FadeInSection delay={0.4}>
          <div className="mt-10 md:mt-20 rounded-2xl border border-primary/20 bg-card p-5 sm:p-6 md:p-10 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-3 md:mb-4">
              ...
              <h2 className="font-display text-lg sm:text-xl md:text-2xl text-primary tracking-wide">Travel & Outstation Experience</h2>
            </div>
            <p className="font-elegant text-xs sm:text-sm md:text-base text-primary/55 leading-relaxed max-w-2xl">
              Companion travel to premium locations, five-star accommodation required, customized itinerary planning.
            </p>
          </div>
        </FadeInSection>
          ...
          <FadeInSection delay={0.5}>
            <div className="rounded-2xl border border-primary/20 bg-card p-5 sm:p-6 md:p-8 h-full relative overflow-hidden">
              <div className="flex items-center gap-3 mb-4 md:mb-5">
                ...
                <h2 className="font-display text-base sm:text-lg md:text-xl text-primary tracking-wide">Exclusive Add-On Privileges</h2>
              </div>
          ...
          <FadeInSection delay={0.6}>
            <div className="rounded-2xl border border-primary/20 bg-card p-5 sm:p-6 md:p-8 h-full relative overflow-hidden">
              <div className="flex items-center gap-3 mb-4 md:mb-5">
                ...
                <h2 className="font-display text-base sm:text-lg md:text-xl text-primary tracking-wide">Booking Guidelines</h2>
              </div>
              <ol className="space-y-2.5 md:space-y-3">
                {["Select your preferred plan", "Choose your desired profile", "Reserve a premium hotel or verified private venue", "Share confirmed date, time, and venue details", "Secure your reservation as per confirmation process"].map((step, i) => (
                  <li key={i} className="flex items-start gap-2.5 md:gap-3 group">
                    <span className="w-5 h-5 md:w-6 md:h-6 rounded-full border border-gold/30 flex items-center justify-center text-[10px] md:text-xs text-gold font-semibold flex-shrink-0 mt-0.5 group-hover:bg-gold/10 transition">{i + 1}</span>
        ...
        <FadeInSection delay={0.7}>
          <div className="mt-8 md:mt-14 text-center max-w-2xl mx-auto">
            <p className="font-elegant text-xs sm:text-sm md:text-base text-primary/45 leading-relaxed italic mb-4">
              All engagements are reserved in advance and allocated on a priority basis.
            </p>
            <div className="gold-divider w-16 mx-auto my-4 md:my-5" />
            <p className="font-display text-xs sm:text-sm md:text-base tracking-[0.15em] md:tracking-[0.2em] text-gold/70">Premium · Private · Precisely Executed</p>
            <p className="font-elegant text-[10px] sm:text-xs md:text-sm text-primary/35 mt-2">Exact pricing shared upon private inquiry. Discretion ensured.</p>
          </div>
        </FadeInSection>
      </div>
    </div>
